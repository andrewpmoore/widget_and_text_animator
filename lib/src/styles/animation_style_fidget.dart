...
class AnimationStyleFidget extends AnimationStyle{
  @override
  AnimationSettings getSettings(WidgetRestingEffects effects, AnimationController animationController) {
    ...
    xList.add(TweenSequenceItem<double>(tween: Tween<double>(begin: 0, end: currentRandom).chain(CurveTween(curve: effects.curve!)),weight: 100/(iterations+2),),);
    for (int i=0; i<iterations; i++){
      nextRandom = _getRandomNumber(min, max, rnd);
      xList.add(TweenSequenceItem<double>(tween: Tween<double>(begin: currentRandom, end: nextRandom).chain(CurveTween(curve: effects.curve!)),weight: 100/(iterations+2),),);
      currentRandom = nextRandom;
    }
    xList.add(TweenSequenceItem<double>(tween: Tween<double>(begin: currentRandom, end: 0).chain(CurveTween(curve: effects.curve!)),weight: 100/(iterations+2),),);

    iterations = (_getRandomNumber(3, 10, rnd)).toInt();
    nextRandom = 0;
    currentRandom = _getRandomNumber(min, max, rnd);
    yList.add(TweenSequenceItem<double>(tween: Tween<double>(begin: 0, end: currentRandom).chain(CurveTween(curve: effects.curve!)),weight: 100/(iterations+2),),);
    for (int i=0; i<iterations; i++){
      nextRandom = _getRandomNumber(min, max, rnd);
      yList.add(TweenSequenceItem<double>(tween: Tween<double>(begin: currentRandom, end: nextRandom).chain(CurveTween(curve: effects.curve!)),weight: 100/(iterations+2),),);
      currentRandom = nextRandom;

    }
    yList.add(TweenSequenceItem<double>(tween: Tween<double>(begin: currentRandom, end: 0).chain(CurveTween(curve: effects.curve!)),weight: 100/(iterations+2),),);


    _animationSettings.offsetXAnimation = TweenSequence<double>(
      <TweenSequenceItem<double>>[
        for (var x in xList)x
      ],
