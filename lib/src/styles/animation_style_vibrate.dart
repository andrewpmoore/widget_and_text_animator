...
class AnimationStyleVibrate extends AnimationStyle{
  @override
  AnimationSettings getSettings(WidgetRestingEffects effects, AnimationController animationController) {
    ...
    for (int i=0; i<iterations; i++){
      ...
      xList.add(TweenSequenceItem<double>(tween: Tween<double>(begin: nextRandom, end: nextRandom).chain(CurveTween(curve: Curves.linear)),weight: _getRandomNumber(10, 100, rnd),),);
    }
    ...
    for (int i=0; i<iterations; i++){
      nextRandom = _getRandomNumber(min, max, rnd);
      yList.add(TweenSequenceItem<double>(tween: Tween<double>(begin: nextRandom, end: nextRandom).chain(CurveTween(curve: Curves.linear)),weight: _getRandomNumber(10, 100, rnd),),);
    }
